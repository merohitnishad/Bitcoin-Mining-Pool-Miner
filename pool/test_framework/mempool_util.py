...
def fill_mempool(test_framework, node):
    ...
    confirmed_utxos = [
        ephemeral_miniwallet.get_utxo(confirmed_only=True)
        for _ in range(num_of_batches * tx_batch_size + 1)
    ]
    ...
    with node.assert_debug_log(["rolling minimum fee bumped"]):
        for batch_of_txid in range(num_of_batches):
            ...
            create_lots_of_big_transactions(
                ephemeral_miniwallet, node, fee, tx_batch_size, txouts, utxos
            )
