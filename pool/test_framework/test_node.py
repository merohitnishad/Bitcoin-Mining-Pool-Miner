...
from .authproxy import JSONRPCException, serialization_fallback
...
class TestNode:
    ...
    def __init__(
        ...
        *,
        ...
        self.args = [
            ...
            "-debugexclude=rand",
            "-uacomment=testnode%d"
            % i,  # required for subversion uniqueness across peers
        ]
        ...
        if use_valgrind and version is None:
            default_suppressions_file = (
                Path(__file__).parents[3] / "contrib" / "valgrind.supp"
            )
    ...
    def get_wallet_rpc(self, wallet_name):
        if self.use_cli:
            return RPCOverloadWrapper(
                self.cli("-rpcwallet={}".format(wallet_name)), True, self.descriptors
            )
        else:
    ...
    def wait_for_new_peer(self, timeout=5):
        ...
        yield
        self.wait_until(
            lambda: get_highest_peer_id() > initial_peer_id, timeout=timeout
        )
        ...
        **kwargs,
        ...
        if wait_for_verack:
            ...
            info = [
                ...
                if peer["addr"] == our_addr_and_port
                and peer["addrbind"] == dst_addr_and_port
            ]
    ...
    def add_outbound_p2p_connection(
        ...
        **kwargs,
        ...
        def addconnection_callback(address, port):
            ...
            self.addconnection(
                "%s:%d" % (address, port), connection_type, advertise_v2_p2p
            )
        ...
        if advertise_v2_p2p:
            kwargs["services"] = kwargs.get("services", P2P_SERVICES) | NODE_P2P_V2
            assert (
                self.use_v2transport
            )  # only a v2 TestNode could make a v2 outbound connection
    ...
    def wait_until(self, test_function, timeout=60):
        return wait_until_helper_internal(
            test_function, timeout=timeout, timeout_factor=self.timeout_factor
        )
