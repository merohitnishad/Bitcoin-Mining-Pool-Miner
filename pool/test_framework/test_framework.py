...
class BitcoinTestFramework(metaclass=BitcoinTestMetaClass):
    ...
    def set_binary_paths(self):
        ...
        for binary, [attribute_name, env_variable_name] in binaries.items():
            ...
            setattr(
                self.options,
                attribute_name,
                os.getenv(env_variable_name, default=default_filename),
            )
    ...
    def init_wallet(self, *, node):
        wallet_name = (
            ...
            else self.wallet_names[node] if node < len(self.wallet_names) else False
        )
    ...
    def add_nodes(
        ...
        *,
        ...
        if binary is None:
            binary = [
                get_bin_from_version(v, "bitcoind", self.options.bitcoind)
                for v in versions
            ]
        if binary_cli is None:
            binary_cli = [
                get_bin_from_version(v, "bitcoin-cli", self.options.bitcoincli)
                for v in versions
            ]
    ...
    def connect_nodes(
        self, a, b, *, peer_advertises_v2=None, wait_for_connect: bool = True
    ):
        """
        Kwargs:
        ...
        if peer_advertises_v2 != from_connection.use_v2transport:
            from_connection.addnode(
                node=ip_port, command="onetry", v2transport=peer_advertises_v2
            )
        else:
