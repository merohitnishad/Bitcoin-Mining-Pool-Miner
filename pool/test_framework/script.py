...
from .key import TaggedHash, compute_xonly_pubkey, tweak_add_pubkey
...
def sign_input_legacy(
    tx, input_index, input_scriptpubkey, privkey, sighash_type=SIGHASH_ALL
):
    ...
    (sighash, err) = LegacySignatureHash(
        input_scriptpubkey, tx, input_index, sighash_type
    )
    assert err is None
...
def sign_input_segwitv0(
    tx, input_index, input_scriptpubkey, input_amount, privkey, sighash_type=SIGHASH_ALL
):
    ...
    sighash = SegwitV0SignatureHash(
        input_scriptpubkey, tx, input_index, sighash_type, input_amount
    )
    der_sig = privkey.sign_ecdsa(sighash)
    tx.wit.vtxinwit[input_index].scriptWitness.stack.insert(
        0, der_sig + bytes([sighash_type])
    )
    tx.rehash()
    ...
    def test_legacy_sigopcount(self):
        ...
        for n_ops in range(1, 100, 10):
            for singlesig_op in (OP_CHECKSIG, OP_CHECKSIGVERIFY):
                ...
                self.assertEqual(
                    singlesigs_script.GetSigOpCount(fAccurate=False), n_ops
                )
